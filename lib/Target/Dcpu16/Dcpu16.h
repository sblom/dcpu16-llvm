...
namespace llvm {
  ...
  FunctionPass *createDcpu16DelaySlotFillerPass(TargetMachine &TM);
  FunctionPass *createDcpu16FPMoverPass(TargetMachine &TM);

} // end namespace llvm;

namespace llvm {
  // Enums corresponding to Dcpu16 condition codes, both icc's and fcc's.  These
  ...
  inline static const char *DCPU16CondCodeToString(SPCC::CondCodes CC) {
    switch (CC) {
