...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that Dcpu16 uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
static bool CC_Dcpu16_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
Dcpu16TargetLowering::LowerFormalArguments(SDValue Chain,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      // Dcpu16 is big endian, so add an offset based on the ObjectVT.
  ...
  for (unsigned i = 0, realArgIdx = 0, byvalArgIdx = 0, e = ArgLocs.size();
    ...
    if (VA.needsCustom()) {
      ...
      SDValue Store = DAG.getStore(DAG.getEntryNode(), dl,
                                   ...
                                   false, false, 0);
      // Dcpu16 is big-endian, so the high part comes first.
...
unsigned
Dcpu16TargetLowering::getSRetArgSize(SelectionDAG &DAG, SDValue Callee) const
{
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a DCPU16 ICC
/// condition.
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a DCPU16
/// FCC condition.
...
Dcpu16TargetLowering::Dcpu16TargetLowering(TargetMachine &TM)
  ...
  // Dcpu16 has no REM or DIVREM operations.
  ...
  // Dcpu16 has no select or setcc: expand to SELECT_CC.
  ...
  // Dcpu16 doesn't have BRCOND either, it has BR_CC.
  ...
  // FIXME: Dcpu16 provides these multiplies, but we don't have them yet.
  ...
  setStackPointerRegisterToSaveRestore(DCPU16::O6);
...
/// combiner.
void Dcpu16TargetLowering::computeMaskedBitsForTargetNode(const SDValue Op,
                                                         APInt &KnownZero,
                                                         APInt &KnownOne,
...
SDValue Dcpu16TargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
...
static SDValue LowerVASTART(SDValue Op, SelectionDAG &DAG,
                            const Dcpu16TargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
  Dcpu16MachineFunctionInfo *FuncInfo = MF.getInfo<Dcpu16MachineFunctionInfo>();
...
LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  ...
  case ISD::GlobalTLSAddress:
    llvm_unreachable("TLS not implemented for Dcpu16.");
...
Dcpu16TargetLowering::EmitInstrWithCustomInserter(MachineInstr *MI,
  ...
  case DCPU16::SELECT_CC_Int_ICC:
  case DCPU16::SELECT_CC_FP_ICC:
  case DCPU16::SELECT_CC_DFP_ICC:
    BROpcode = DCPU16::BCOND;
    break;
  case DCPU16::SELECT_CC_Int_FCC:
  case DCPU16::SELECT_CC_FP_FCC:
  case DCPU16::SELECT_CC_DFP_FCC:
    BROpcode = DCPU16::FBCOND;
    break;
  }
  ...
  //  ...
  BB = sinkMBB;
  BuildMI(*BB, BB->begin(), dl, TII.get(DCPU16::PHI), MI->getOperand(0).getReg())
...
Dcpu16TargetLowering::ConstraintType
...
bool
Dcpu16TargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The Dcpu16 target isn't yet aware of offsets.
  return false;
}
