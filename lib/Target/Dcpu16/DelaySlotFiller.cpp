...
static cl::opt<bool> DisableDelaySlotFiller(
  "disable-dcpu16-delay-filler",
  cl::init(false),
  cl::desc("Disable the Dcpu16 delay slot filler."),
  cl::Hidden);
...
/// createDcpu16DelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Dcpu16 MachineFunctions
///
FunctionPass *llvm::createDcpu16DelaySlotFillerPass(TargetMachine &tm) {
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(); I != MBB.end(); ++I)
    if (I->hasDelaySlot()) {
      ...
      if (D == MBB.end())
        BuildMI(MBB, ++J, I->getDebugLoc(), TII->get(DCPU16::NOP));
      else
        MBB.splice(++J, &MBB, D);
      ...
      if (needsUnimp(I, structSize)) {
        ...
        BuildMI(MBB, ++J, I->getDebugLoc(),
                TII->get(DCPU16::UNIMP)).addImm(structSize);
      }
    }
  return Changed;
}
                       ...
                       MachineBasicBlock::iterator slot)
{
  ...
  if (slot->getOpcode() == DCPU16::RET)
    return MBB.end();

  if (slot->getOpcode() == DCPU16::RETL) {
    --I;
    if (I->getOpcode() != DCPU16::RESTORErr)
      return MBB.end();
    //change retl to ret
    slot->setDesc(TII->get(DCPU16::RET));
    return I;
  }
                           ...
                           MachineBasicBlock::iterator candidate)
{
  if (candidate == MBB.begin())
    return false;
  if (candidate->getOpcode() == DCPU16::UNIMP)
    return true;
  --candidate;
...
bool Filler::needsUnimp(MachineBasicBlock::iterator I, unsigned &StructSize)
{
  ...
  case DCPU16::CALL: structSizeOpNum = 1; break;
  case DCPU16::JMPLrr:
  case DCPU16::JMPLri: structSizeOpNum = 2; break;
  }
