...
// of machine-dependent LLVM code to GAS-format DCPU16 assembly language.
//
//===----------------------------------------------------------------------===//
...
namespace {
  ...
  public:
    explicit Dcpu16AsmPrinter(TargetMachine &TM, MCStreamer &Streamer)
...
void Dcpu16AsmPrinter::printOperand(const MachineInstr *MI, int opNum,
  ...
  if (MI->getOpcode() == DCPU16::SETHIi && !MO.isReg() && !MO.isImm()) {
    O << "%hi(";
    CloseParen = true;
  } else if ((MI->getOpcode() == DCPU16::ORri || MI->getOpcode() == DCPU16::ADDri) &&
             !MO.isReg() && !MO.isImm()) {
    O << "%lo(";
    CloseParen = true;
  }*/
